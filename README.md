...
**清零时机:**
- ✅ 程序启动时,如果CSV文件满足以下任一条件,CVD从0开始:
  - CSV文件不存在
  - CSV文件为空
  - CSV文件太旧(超过 `cvd_max_age_days` 天,默认6天)
  - CSV文件解析失败
- ❌ 程序运行期间**不会清零**,持续累积

**配置参数:** `settings.yaml` 中的 `cvd_reset.max_age_days`
...
```python
# 对每笔交易:
period_volume += quantity  # 累加交易量(不区分买卖方向)
```

**清零时机:**
- ✅ 每次保存数据后自动清零
- ✅ 清零频率: 根据配置的 `save_interval_minutes` 参数(默认1分钟)

**配置参数:** `settings.yaml` 中的 `data_saving.interval_minutes`
...
| 指标 | 计算方式 | 清零时机 | 清零频率 | 持久化 |
|------|---------|---------|---------|--------|
| **cvd** | 买入量-卖出量累积 | 程序启动时(条件性) | 文件超过6天 | ✅ 是 |
| **period_volume** | 交易量累加 | 每次保存数据后 | 默认1分钟 | ✅ 是 |
| **trade_count** | 交易笔数计数 | 永不清零 | - | ✅ 是 |

## 数据输出

程序会将CVD数据保存到 `cvd_data_optimized/` 目录下，根据配置的写入模式：
- 单文件模式：`cvd_data_all.csv`
- 多文件模式：按符号分文件 `{SYMBOL}_{TYPE}.csv`

**CSV文件格式:**
- 单文件: `timestamp,symbol,price,cvd,period_volume,trade_count`
- 多文件: `timestamp,price,cvd,period_volume,trade_count`
