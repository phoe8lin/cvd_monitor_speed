...
use_single_file = True  # 默认使用单文件模式
...
try:
    with open(SETTINGS_FILE, 'r') as f:
        ...
        use_single_file = settings.get('data_saving', {}).get('use_single_file', use_single_file)
...
# --- 从CSV加载最后的CVD值 ---
def load_last_cvd_from_unified_csv(csv_file_path, symbol_key, max_age_days=1):
    """
    从统一的CSV文件加载指定符号的最后CVD值
    ...
    try:
        ...
        if (now - file_mtime_dt).days > max_age_days:
            logger.info(f"CSV文件 {csv_file_path} 太旧（{(now - file_mtime_dt).days} 天）。{symbol_key} 将从0开始CVD计算。")
            return 0.0
        ...
        with open(csv_file_path, 'r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                if row.get('symbol') == symbol_key:
                    try:
                        last_cvd = float(row.get('cvd', 0.0))
                    except (ValueError, TypeError):
                        pass

        if last_cvd != 0.0:
            logger.info(f"从统一CSV文件加载了 {symbol_key} 的最后CVD值: {last_cvd}")
        return last_cvd

    except Exception as e:
        logger.error(f"从统一CSV文件加载CVD时发生错误: {e}。将从0开始CVD计算。")
        return 0.0
...
class SymbolCvdMonitor:
    ...
    def __init__(self, symbol_config, shared_session, data_store, data_dir, use_single_file=True):
        """初始化 CVD 监控器"""
        ...
        # 根据模式加载初始CVD
        if use_single_file:
            csv_file_path = os.path.join(data_dir, "cvd_data_all.csv")
            initial_cvd = load_last_cvd_from_unified_csv(csv_file_path, self.shared_key, cvd_max_age_days)
        else:
...
# --- 数据保存功能 ---
async def save_cvd_data_unified(data_store, monitors, csv_file_path):
    """
    将所有符号的CVD数据保存到统一的CSV文件
    CSV格式: timestamp,symbol,price,cvd,period_volume
    """
    ...
    for key, monitor in monitors.items():
        ...
        rows_to_save.append([timestamp, key, last_price, cvd, period_volume])
        saved_count += 1
    ...
    if rows_to_save:
        ...
        with open(csv_file_path, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(['timestamp', 'symbol', 'price', 'cvd', 'period_volume'])
            writer.writerows(rows_to_save)
    
    for monitor in monitors.values():
        monitor.reset_period_volume()
    
    logger.info(f"[单文件模式] 已保存 {saved_count} 个符号的CVD数据")

async def save_cvd_data_separate(data_store, monitors, data_dir):
    """
    将每个符号的CVD数据保存到独立的CSV文件 (优化版:批量写入)
    CSV格式: timestamp,price,cvd,period_volume
    """
    ...
    for key, monitor in monitors.items():
        ...
        files_to_write[csv_file] = [timestamp, last_price, cvd, period_volume]
        saved_count += 1
    
    # 批量写入文件
    for csv_file, row_data in files_to_write.items():
        try:
            ...
            with open(csv_file, 'a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                if not file_exists:
                    writer.writerow(['timestamp', 'price', 'cvd', 'period_volume'])
    ...
    logger.info(f"[多文件模式] 已保存 {saved_count} 个符号的CVD数据到独立文件")

async def data_saver_task(data_store, monitors, data_dir, interval_seconds, shutdown_event, use_single_file=True):
    """数据保存协程任务"""
    mode_name = "单文件" if use_single_file else "多文件"
    logger.info(f"启动数据保存任务({mode_name}模式)，间隔: {interval_seconds} 秒")
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if not shutdown_event.is_set():
                ...
                if use_single_file:
                    csv_file_path = os.path.join(data_dir, "cvd_data_all.csv")
                    await save_cvd_data_unified(data_store, monitors, csv_file_path)
                else:
                    await save_cvd_data_separate(data_store, monitors, data_dir)
...
async def main():
    """主异步函数"""
    mode_name = "单文件" if use_single_file else "多文件"
    logger.info(f"初始化 CVD 监视器 ({mode_name}模式)...")
    ...
    logger.info(f"\n--- 已创建 {len(monitors)} 个监控器 ({mode_name}模式) ---")
    ...
    saver_task = asyncio.create_task(
        data_saver_task(data_store, monitors, DATA_DIR, save_interval_seconds, shutdown_event, use_single_file)
    )
